...
export async function generateMetadata({ params, searchParams }) {
  ...
  const title =
    wordCount == 1
      ? `Words Ending In ${upperWord}`
      : `${wordCount} Letter Words Ending in ${upperWord}`;

  return {
    title: title + " - The Word Mixer",
    description: `Searching words ending in ${upperWord}? view results of over 100 words that end in ${upperWord}`,
    openGraph: {
      title: title + " - The Word Mixer",
      type: "article",
      description: `Searching words ending in ${upperWord}? view results of over 100 words that end in ${upperWord}`,
      url: `https://thewordmixer.com/words-ending-in/${word.trim()}`,
...
async function getEndingWithWords(wordCount, trimmedWord) {
  let url;
  if (wordCount === 1) {
    ...
    url = `https://api.datamuse.com/words?sp=${questionMarkCount}${trimmedWord}&md=d&max=100`;
  }
...
export default async function WordsEndingInPage({ params, searchParams }) {
  ...
  let results;
  try {
    results = await getEndingWithWords(wordCount, trimmedWord);
  } catch (error) {
    results = [];
  }
